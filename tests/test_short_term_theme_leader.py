...
def test_ths_name_list_without_constituents_is_not_theme_data(monkeypatch):
    module = _module()
    responses = {
        "stock_board_concept_name_em": ([], "failed"),
        "stock_board_concept_name_ths": [{"名称": f"概念{i}"} for i in range(375)],
        "stock_board_industry_name_em": ([], "failed"),
        "stock_board_industry_name_ths": [{"行业名称": "通信设备"}],
    }
    market = _market(module, [{"代码": "000101", "名称": "甲", "所属行业": "通信设备", "连板数": 2}])
    _fake_universe_fetch(monkeypatch, module, responses)
    universe = module._fetch_theme_universe("20260817", market, {})
    assert universe["concept_available"] is True
    assert universe["concept_mapping_available"] is False
    assert universe["source_mode"] == "INDUSTRY_DEGRADED"
    assert set(universe["themes"]) == {"通信设备"}


def test_empty_themes_are_excluded_from_main_theme_candidates():
    module = _module()
    themes = {
        "空板块": {"theme_name": "空板块", "members": [], "member_rows": [], "concept_available": False, "data_quality": "partial"},
        "通信设备": {
            "theme_name": "通信设备", "theme_type": "INDUSTRY_DEGRADED", "source": "limit_up_industry_cluster",
            "members": ["000102"], "member_rows": [{"代码": "000102", "所属行业": "通信设备"}],
            "concept_available": True, "data_quality": "degraded", "is_degraded": True,
        },
    }
    market = _market(module, [{"代码": "000102", "所属行业": "通信设备", "连板数": 3}])
    metrics = module._build_theme_metrics(themes, {}, {"000102": ["通信设备"]}, market)
    assert metrics["空板块"]["is_actionable_theme"] is False
    assert metrics["通信设备"]["is_actionable_theme"] is True


def test_degraded_theme_detection_valid_requires_real_strength():
    module = _module()
    assert module._theme_detection_valid({"空": {"is_actionable_theme": False, "limit_up_count": 0, "confidence_adjusted_score": 0}}) is False
    assert module._theme_detection_valid({"真": {"is_actionable_theme": True, "limit_up_count": 1, "confidence_adjusted_score": 1}}) is True


def test_full_industry_name_is_preserved_without_fixed_length_truncation():
    module = _module()
    assert module._theme_name({"行业名称": "房地产开发"}) == "房地产开发"
    assert module._theme_name({"行业": "农产品加工"}) == "农产品加工"


def test_degraded_pool_binds_leader_to_industry_theme(monkeypatch):
    module = _module()
    responses = {
        "stock_board_concept_name_em": ([], "failed"),
        "stock_board_concept_name_ths": ([], "failed"),
    ...
    market = _market(module, [{"代码": "000103", "名称": "己", "所属行业": "房地产开发", "连板数": 4, "首次封板时间": "09:35"}])
    _fake_universe_fetch(monkeypatch, module, responses)
    universe = module._fetch_theme_universe("20260817", market, {})
    metrics = module._build_theme_metrics(universe["themes"], {}, universe["stock_to_themes"], market)
    records = module._build_stock_records({}, universe["stock_to_themes"], universe["stock_to_industries"], metrics, market)
    assert records["000103"]["primary_theme"] == "房地产开发"
    assert records["000103"]["primary_theme_role"] is not None


def test_industry_degraded_mode_has_actionable_main_theme_and_market_leader(monkeypatch):
    module = _module()
    responses = {
        "stock_board_concept_name_em": ([], "failed"),
        "stock_board_concept_name_ths": [{"名称": "空概念"}],
        "stock_board_industry_name_em": ([], "failed"),
        "stock_board_industry_name_ths": [{"行业名称": "电网设备"}],
    }
    rows = [{"代码": f"0002{i:02d}", "名称": f"股{i}", "所属行业": "电网设备", "连板数": 5 if i == 1 else 1, "首次封板时间": "09:35"} for i in range(1, 11)]
    market = _market(module, rows, hot=rows[:2])
    candidates = {"000201": {"code": "000201", "resonance_count": 3}}
    _fake_universe_fetch(monkeypatch, module, responses)
    universe = module._fetch_theme_universe("20260817", market, candidates)
    metrics = module._build_theme_metrics(universe["themes"], candidates, universe["stock_to_themes"], market)
    records = module._build_stock_records(candidates, universe["stock_to_themes"], universe["stock_to_industries"], metrics, market)
    _, market_leaders = module._build_leaders(records, metrics)
    assert universe["source_mode"] == "INDUSTRY_DEGRADED"
    assert module._theme_detection_valid(metrics) is True
    assert any(item["theme_name"] == "电网设备" and item["is_actionable_theme"] for item in metrics.values())
    assert market_leaders
"""Pure-rule tests for the stage-3 theme and leader layer."""
